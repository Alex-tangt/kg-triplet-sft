#!/usr/bin/env python3
"""Train the GraphRAG FULL set (2575 rows x 5 epochs) with LLaMA-Factory on Kaggle.
...
size 2). Data is the H4-rebuilt full alpaca (trace-full-dataset, 2575 train),
unified-field prompt, byte-identical to the tracer's except scale.

FP16 (not bf16): T4 has no bf16 tensor core, bf16 falls back to FP32 GEMMs
(measured 3.3x slower). The fp16-vs-bf16 risk was cleared on the tracer
(fp16 loss 0.6595 vs bf16 0.6667). Loss is logged per 5 steps for drift checks.
Estimated ~7h on dual T4 (fp16 15.8s/opt-step, mean seq ~2700 tok); checkpoints
saved each epoch (save_strategy epoch, total_limit 3) for resume if needed.

Run: python /kaggle/src/script.py   (metadata in kernel_lffull/)
"""
...
TRAIN_SRC = os.path.join(DS_DIR, "trace-full-dataset/alpaca_full_train.jsonl")
...
def write_yaml() -> None:
    log("=== 3. full yaml (2575 rows, 5 epochs, dual-GPU DDP) ===")
    cfg = {
        ...
        "save_total_limit": 3,
        "report_to": "none",
        "output_dir": os.path.join(WORK, "saves/qwen3-0.6b-graphrag-full-lf"),
...
def run_train() -> None:
    ...
    out_dir = os.path.join(WORK, "saves/qwen3-0.6b-graphrag-full-lf")
    ckpts = sorted(
