...
(sample_200_passages.jsonl) with the STUDENT_PROMPT (thinking OFF) — the exact
protocol of kernel_inferfull (chat template + schema-prefix injection + tolerant
parser + length-sorted batched decode + one worker per GPU), so capacity models
are directly comparable to the reference baseline (entity recall 0.541).
...
MODEL_SLUG = "qwen-lm/qwen-3/transformers/1.7b/1"          # e.g. qwen-lm/qwen-3/transformers/0.6b/1
ADAPTER_NAME = "qwen3-1.7b"      # e.g. qwen3-0.6b
