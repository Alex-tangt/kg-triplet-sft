...
Runs on a Kaggle T4. Pipeline:
  1. Verify environment (python/torch/GPU).
  2. Install llama-factory==0.9.5 (pinned) if missing.
  3. Stage /kaggle/working/data: dataset_info.json (absolute file paths into
     the mounted datasets) + the train yaml (model mounted from Kaggle Models,
     template qwen3_nothink -> no thinking tokens).
  4. llamafactory-cli train.
  5. Inference on the 10 tracer-eval passages (embedded in inputs.py),
     single-shot, thinking OFF, parse each raw completion, write
     /kaggle/working/predictions.jsonl as {"id", "entities", "relationships"}.

Every path is absolute and checked before use. No interactive steps.

Mode: VERIFY (default when --verify) parses the staged yaml through
get_train_args and exits — no training, CPU-only, for pre-flight validation.
Mode: FULL trains then infers.
"""
...
YAML_PATH = os.path.join(DATA, "tracer.yaml")
...
def install() -> None:
    log("=== 2. install llama-factory ===")
    try:
        import llamafactory  # noqa: F401

        log(f"llamafactory already installed: {llamafactory.__version__}")
        return
    except ImportError:
        pass
    subprocess.check_call(
        [sys.executable, "-m", "pip", "install", "--quiet", "llamafactory==0.9.5"]
    )
    import llamafactory  # noqa: F401

    log(f"installed llamafactory {llamafactory.__version__}")


def stage_data() -> None:
    log("=== 3. stage dataset_info.json + yaml ===")
    info = {
        "graphrag_tracer": {
            "file_name": TRAIN_SRC,
    ...
    with open(os.path.join(DATA, "dataset_info.json"), "w", encoding="utf-8") as f:
    ...
    with open(YAML_PATH, "w", encoding="utf-8") as f:
        f.write(yaml)
    log(f"dataset_info.json + yaml written to {DATA}")


def verify() -> None:
    """Parse the staged yaml through llama-factory's arg parser (no training).

    Unknown keys would raise here; also prints the resolved key settings.
    """
    log("=== 3b. VERIFY: parse staged yaml ===")
    import yaml

    with open(YAML_PATH, "r", encoding="utf-8") as f:
        raw = yaml.safe_load(f)
    # VERIFY runs on a CPU kernel: bf16/GPU semantics can't be satisfied, but
    # the point here is schema/unknown-key validation, so neutralise it.
    raw.pop("bf16", None)
    raw["use_cpu"] = True
    log(f"yaml keys: {sorted(raw.keys())}")

    from llamafactory.hparams import get_train_args

    _model_args, _data_args, _training_args, _finetune_args, _generating_args = get_train_args(raw)
    log(f"VERIFY OK: template resolved, lora_rank={_finetune_args.lora_rank}, "
        f"cutoff_len={_data_args.cutoff_len}, enable_thinking={_data_args.enable_thinking}, "
        f"dataset={_data_args.dataset}, stage={_finetune_args.stage}")
...
def train() -> None:
    ...
    log(f"training finished -> {ADAPTER_DIR}")


def _parse_answer(raw: str) -> dict:
    """Pull the JSON object out of a raw completion (strip fences/leading text)."""
    import re as _re
...
def infer() -> None:
    log("=== 5. inference on 10 eval passages ===")
    ...
    for i, p in enumerate(INPUTS["passages"], 1):
        ...
        try:
            ...
            log(f"[{i}/{len(INPUTS['passages'])}] {p['id']}: PARSE FAIL ({e})")
            parsed = {"entities": [], "relationships": []}
        rows.append(
            {"id": p["id"], "entities": parsed.get("entities", []),
             "relationships": parsed.get("relationships", [])}
        )
        log(f"[{i}/{len(INPUTS['passages'])}] {p['id']}: "
            f"entities={len(rows[-1]['entities'])} rels={len(rows[-1]['relationships'])}")
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--verify", action="store_true", help="pre-flight yaml/env validation only")
