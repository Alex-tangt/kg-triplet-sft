"""Official Microsoft GraphRAG extraction prompts, verbatim (MIT License).

Source: microsoft/graphrag, ``packages/graphrag/graphrag/prompts/index/extract_graph.py``
(https://github.com/microsoft/graphrag/blob/main/packages/graphrag/graphrag/prompts/index/extract_graph.py),
fetched from the repository on 2026-09-01. Copyright (c) 2024 Microsoft Corporation.

The extraction prompt takes ``{entity_types}`` and ``{input_text}`` placeholders.
``CONTINUE_PROMPT`` and ``LOOP_PROMPT`` drive the completeness loop: after the
initial extraction the model is asked whether entities/relationships were
missed (loop), and if so to emit them (continue), until it answers N.
